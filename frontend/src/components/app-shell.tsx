...
type IconName = "dashboard" | "profile" | "world" | "logout" | "headset" | "spark";
...
const roleConfig = {
  ...
  DOCENTE: {
    label: "Docente",
    homeLabel: "Mi observatorio",
    descriptor: "Mentor del grupo",
  },
  ADMINISTRADOR: {
    label: "Administrador",
    homeLabel: "Centro de control",
...
function ShellIcon({ name, className = "h-4 w-4" }: { name: IconName; className?: string }) {
  ...
  if (name === "dashboard") {
    return <svg {...common}><rect height="7" rx="2" width="7" x="3" y="3" /><rect height="7" rx="2" width="7" x="14" y="3" /><rect height="7" rx="2" width="7" x="3" y="14" /><path d="M14 17.5h7M17.5 14v7" /></svg>;
  }
  if (name === "profile") {
    return <svg {...common}><circle cx="12" cy="8" r="3.4" /><path d="M5.5 20c.6-4 2.8-6 6.5-6s5.9 2 6.5 6" /></svg>;
  }
  if (name === "world") {
    return <svg {...common}><circle cx="12" cy="12" r="8.5" /><path d="M3.8 12h16.4M12 3.5c2.3 2.5 3.5 5.3 3.5 8.5S14.3 18 12 20.5C9.7 18 8.5 15.2 8.5 12S9.7 6 12 3.5Z" /></svg>;
  }
  if (name === "logout") {
    return <svg {...common}><path d="M10 5H5.5A1.5 1.5 0 0 0 4 6.5v11A1.5 1.5 0 0 0 5.5 19H10M14.5 8l4 4-4 4M9 12h9" /></svg>;
  }
  if (name === "headset") {
    return <svg {...common}><path d="M4 13v-2a8 8 0 0 1 16 0v2M5.5 12H7a1 1 0 0 1 1 1v5H6.5A2.5 2.5 0 0 1 4 15.5v-1A2.5 2.5 0 0 1 6.5 12ZM18.5 12H17a1 1 0 0 0-1 1v5h1.5a2.5 2.5 0 0 0 2.5-2.5v-1a2.5 2.5 0 0 0-2.5-2.5Z" /></svg>;
  }
  return <svg {...common}><path d="m12 3 1.3 4.2a5.3 5.3 0 0 0 3.5 3.5L21 12l-4.2 1.3a5.3 5.3 0 0 0-3.5 3.5L12 21l-1.3-4.2a5.3 5.3 0 0 0-3.5-3.5L3 12l4.2-1.3a5.3 5.3 0 0 0 3.5-3.5L12 3Z" /></svg>;
}
...
export function AppShell({ usuario, children, eyebrow, title }: Props) {
  ...
  const links = [
    { href: home, label: config.homeLabel, icon: "dashboard" as const },
    { href: `${home}#perfil`, label: "Mi identidad", icon: "profile" as const },
    ...
    <main className="app-surface min-h-screen overflow-x-hidden text-slate-100">
      <div className="mx-auto grid min-h-screen max-w-[1760px] lg:grid-cols-[292px_minmax(0,1fr)]">
        <aside className="relative z-20 border-b border-white/10 bg-[#05100e]/90 px-4 py-4 shadow-[0_20px_70px_rgba(0,0,0,.18)] backdrop-blur-2xl lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col lg:border-b-0 lg:border-r lg:px-5 lg:py-6">
          <div aria-hidden="true" className="pointer-events-none absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-emerald-200/60 to-transparent" />
          <Link className="group flex items-center gap-3 rounded-2xl p-2 transition hover:bg-white/[.035]" href="/">
            <span className="brand-mark transition duration-300 group-hover:rotate-3 group-hover:scale-105">A</span>
            <span className="min-w-0">
              ...
              <small className="flex items-center gap-1.5 text-[9px] uppercase tracking-[.25em] text-emerald-300/75">
                <span className="h-1 w-1 rounded-full bg-emerald-300 shadow-[0_0_8px_#6ee7b7]" /> Portal MR activo
              </small>
            </span>
          </Link>

          <div className="mt-4 flex items-center justify-between px-2 lg:mt-9">
            <span className="font-mono text-[9px] uppercase tracking-[.22em] text-slate-600">Navegación</span>
            <span className="rounded-full border border-white/10 px-2 py-1 font-mono text-[8px] text-slate-500">WEB.02</span>
          </div>
          <nav aria-label="Navegación principal" className="mt-2 flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible">
            {links.map((link) => {
              const active = link.href === home && pathname === home;
              return (
                <Link
                  ...
                  key={link.href}
                >
                  <span className={`grid h-8 w-8 place-items-center rounded-lg transition ${active ? "bg-emerald-300/10 text-emerald-300" : "bg-white/[.035] text-slate-500 group-hover:text-emerald-200"}`}>
                    <ShellIcon name={link.icon} />
                  </span>
                  <span>{link.label}</span>
                  {active ? <span className="ml-auto hidden h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_10px_#6ee7b7] lg:block" /> : null}
                </Link>
              );
            })}
          </nav>
          ...
          <div className="mt-7 hidden overflow-hidden rounded-[1.35rem] border border-cyan-200/10 bg-gradient-to-br from-cyan-300/[.07] to-emerald-300/[.025] p-4 lg:block">
            <div className="flex items-start justify-between gap-3">
              <span className="grid h-9 w-9 place-items-center rounded-xl border border-cyan-200/15 bg-cyan-200/[.07] text-cyan-200">
                <ShellIcon name="headset" className="h-5 w-5" />
              </span>
            ...
            <p className="mt-1 text-[11px] leading-5 text-slate-500">Perfil, progreso e informes comparten el mismo estado de aprendizaje.</p>
          </div>
            ...
            <div className="rounded-[1.35rem] border border-white/10 bg-white/[.035] p-3.5">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <span className="min-w-0 flex-1">
                  <strong className="block truncate text-sm text-slate-100">{usuario?.nombre ?? "Sincronizando…"}</strong>
                  <small className="mt-0.5 block truncate text-[10px] text-emerald-300/80">{usuario ? config.descriptor : ""}</small>
                </span>
              </div>
              <button
        ...
        <section className="relative z-10 min-w-0 px-4 py-5 sm:px-7 lg:px-9 lg:py-8 xl:px-12">
          <header className="relative mb-7 overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#071713]/65 px-5 py-5 shadow-[0_26px_80px_rgba(0,0,0,.18)] backdrop-blur-xl sm:px-6 lg:mb-8">
            <div aria-hidden="true" className="absolute -right-20 -top-28 h-64 w-64 rounded-full bg-emerald-300/[.07] blur-3xl" />
            <div aria-hidden="true" className="absolute right-[18%] top-0 h-px w-40 bg-gradient-to-r from-transparent via-cyan-200/50 to-transparent" />
            <div className="relative flex flex-wrap items-end justify-between gap-5">
              <div className="flex min-w-0 items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="section-kicker flex items-center gap-2"><ShellIcon name="spark" /> {eyebrow}</p>
                  <h1 className="mt-2 truncate text-3xl font-semibold tracking-[-.045em] sm:text-4xl">{title}</h1>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-2 rounded-full border border-emerald-300/15 bg-emerald-300/[.06] px-3 py-2 text-[10px] text-emerald-100/75">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_12px_#6ee7b7]" /> Datos sincronizados
                </span>
              </div>
            </div>
          </header>
